...
impl TemplateApp {
    ...
    fn extract_file_from_archive(&mut self, archive_path: &str, file_name: &str) -> Result<PathBuf> {
        ...
        for folder_index in 0..folder_count {
            let mut file = File::open(archive_path)?;
            let folder_dec = BlockDecoder::new(folder_index, &archive, password.as_slice(), &mut file);
            ...
            // 创建或获取现有的临时目录
            let temp_dir = self.temp_dir.get_or_insert_with(|| tempdir().expect("Failed to create temp dir"));
    ...
    fn load_texture_from_path(&mut self, ctx: &egui::Context, path: &PathBuf) -> Option<TextureHandle> {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if let Some(texture) = &self.texture {
            egui::SidePanel::right("image_panel").show(ctx, |ui| {
                ...
                let (final_width, final_height) = if original_width <= max_width && original_height <= max_height {
            ...
            ui.horizontal(|ui| {
                if ui.button("Select File").clicked() {
                    if let Some(file) = FileDialog::new().pick_file() {
                        ...
                        self.list_files_in_archive(file.to_str().unwrap()).expect("Failed to list files");
                    }
                }
                    ...
                    ui.horizontal(|ui| {
                        if ui.button(select_file).clicked() {
                            ...
                            if let Some(archive_path) = &selected_file {
                                let extracted_path = self.extract_file_from_archive(
                                    archive_path.to_str().unwrap(),
                                    select_file,
                                ).expect("Failed to extract file");
                                self.print_choose_file(select_file);
                                println!("Extracted to: {}", extracted_path.to_str().unwrap_or("Invalid path"));
                                if select_file.ends_with(".png") || select_file.ends_with(".jpg") || select_file.ends_with(".jpeg") {
                                    self.texture = self.load_texture_from_path(ctx, &extracted_path);
                                }
